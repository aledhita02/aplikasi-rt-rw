	...
	<form action="" method="post" enctype="multipart/form-data">
		<div class="card-body">
			...
			<div class="form-group row">
				...
				<div class="col-sm-8">
					<input type="text" class="form-control" value="<?php echo $data_cek['alamat']; ?>, RT <?php echo $data_cek['rt']; ?> / RW <?php echo $data_cek['rw']; ?>, Desa <?php echo $data_cek['desa']; ?> (<?php echo $data_cek['kec']; ?> - <?php echo $data_cek['kab']; ?> - <?php echo $data_cek['prov']; ?>)"
					 readonly/>
				</div>
			</div>
				...
				<div class="table-responsive">
					<table class="table table-bordered table-striped table-hover">
						...
						<tbody>
							...
							<?php
              $no = 1;
			  $sql = $koneksi->query("SELECT p.nik, p.nama, p.jekel, p.alamat, a.hubungan, a.id_anggota 
		...
		<div class="card-footer">
			<a href="?page=data-kartu" title="Kembali" class="btn btn-primary">Kembali</a>
		</div>
	</form>
</div>
...
if (isset($_POST['Simpan'])) {
    ...
    $sql_cek = "SELECT * FROM tb_anggota WHERE id_kk = '" . $_POST['id_kk'] . "' AND id_pend = '" . $_POST['id_pend'] . "'";
    $query_cek = mysqli_query($koneksi, $sql_cek);
    $jumlah_data = mysqli_num_rows($query_cek);
    ...
    if ($jumlah_data > 0) {
        ...
        echo "<script>
            Swal.fire({
                title: 'Error !',
                text: 'Data anggota sudah ada',
                icon: 'error',
                confirmButtonText: 'OK'
            });
        </script>";
    } else {
        // Data belum ada, lanjutkan proses penyimpanan data
